...
function AtNeedContractDetailPage() {
	...
	const signMutation = useMutation({
		mutationFn: () => atNeedContractApi.signAtNeedContract(contractId),
		onSuccess: () => {
	...
	const cancelMutation = useMutation({
		mutationFn: () =>
			atNeedContractApi.cancelAtNeedContract(contractId, "Cancelled by user"),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["at-need-contract", contractId],
			});
	...
	const deleteMutation = useMutation({
		mutationFn: () => atNeedContractApi.deleteAtNeedContract(contractId),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["at-need-contracts"] });
	...
	return (
		<div className="mx-auto max-w-4xl space-y-8">
			...
			<div className="rounded-lg border bg-card p-6">
				<h2 className="text-lg font-semibold">Deceased Information</h2>
				<div className="mt-4 space-y-4">
					<div className="flex items-center justify-between rounded-md border bg-background p-4">
						<div className="space-y-1">
							<div className="font-medium">{contract.deceasedName}</div>
							<div className="text-sm text-muted-foreground">
								Date of Death: {formatDate(contract.dateOfDeath)}
							</div>
						</div>
					</div>
				</div>
